...
private let logger = Logger(subsystem: "com.mtkg.kuntraykun", category: "SelfUpdater")
...
final class SelfUpdater {
    ...
    func performUpdate(to release: ReleaseInfo) async throws {
        ...
        let workDir = fm.temporaryDirectory
            .appendingPathComponent("kuntraykun-update-\(UUID().uuidString)", isDirectory: true)
        ...
        // ローカルビルド（`.local` サフィックス）でも本番リリースへ更新できるよう、基底IDで比較する。
    ...
    /// 末尾の `.local`（ローカル検証ビルド）を取り除いた基底バンドルID。
    private static func baseBundleID(_ id: String?) -> String {
