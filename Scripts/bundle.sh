...
#   本番アプリ(com.mtkg.kuntraykun)と権限/設定が衝突しないよう、バンドルID と表示名を分けた
#   「Kuntraykun (Local)」を生成する。メニューバーには「ローカル」と併記され本番と区別できる。
#
# 署名:
#   既定では Developer ID Application（team id 指定）で署名する。
#   安定した署名アイデンティティにすると、再ビルドや更新で .app を入れ替えても権限が保持される。
#   環境変数で上書きできる:
...
# ローカル検証ビルドはバンドルID/表示名を分けて本番と分離する。
if [[ "$LOCAL" == "1" ]]; then
  APP_NAME="Kuntraykun (Local)"
  BUNDLE_ID="com.mtkg.kuntraykun.local"
else
  APP_NAME="Kuntraykun"
  BUNDLE_ID="com.mtkg.kuntraykun"
fi
...
cp "$BIN_DIR/Kuntraykun" "$APP/Contents/MacOS/Kuntraykun"
cp "$ROOT/Resources/Info.plist" "$APP/Contents/Info.plist"

# ローカル検証ビルドはバンドルID/表示名を差し替える（CFBundleExecutable は Kuntraykun のまま）。
if [[ "$LOCAL" == "1" ]]; then
...
RES_BUNDLE="$BIN_DIR/Kuntraykun_Kuntraykun.bundle"
